...
VERSION=$(TMP=$(brew info audiorecorder | grep -Eo "/audiorecorder/.* \(") ; echo ${TMP:15:(${#TMP}-17)})
config="${HOME}/.electron-app/audiorecordergui.json"
...
sample_rate=$(cat "${config}" | awk -F\" '{print $10}')
bit_depth=$(cat "${config}" | awk -F\" '{print $6}')
channel_layout=$(cat "${config}" | awk -F\" '{print $14}')
output=$(cat "${config}" | awk -F\" '{print $18}')
ITEM_ID=$(cat "${config}" | awk -F\" '{print $22}')
